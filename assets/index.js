...
var WikiSearch = function(){
  ...
  var previousSearch = '',
    ...
    return {
      ...
      searchPageTitle: function(pageTitles, pageTitleIndex, len, isNewSearch){
        var thisWikiSearch = this;
        ...
        $.ajax({
          ...
          success: function(detailData){
            ...
            if (pageTitleIndex != len-1){
              thisWikiSearch.searchPageTitle(pageTitles, pageTitleIndex+1, len);
            } else {
      ...
      searchPageTitles: function(pageTitles, isNewSearch){
        ...
        this.searchPageTitle(pageTitles, pageTitleIndex, len, isNewSearch);
      },

      //gets accurate search results as page titles
      searchWiki: function(searchTerm, isNewSearch){
        var thisWikiSearch = this;
        ...
        $.ajax({
          ...
          success: function(data){
            ...
            if (pages.length != 0){
              ...
              thisWikiSearch.searchPageTitles(pageTitles, isNewSearch);
            } else {
              $('#loading-new').hide();
              $('#no-results').show();
            }
...
$(document).ready(function(){
  var myWikiSearch = new WikiSearch();

  $('#btn-search').on('click', function(e){
    var searchTxt = $('#txt-search').val();
    if(searchTxt != ''){
      $('#txt-search').autocomplete('close');
      myWikiSearch.searchWiki(searchTxt, true);
  ...
  $('#btn-more').on('click', function(e){
    myWikiSearch.searchWiki(myWikiSearch.getPreviousSearch(), false);           
  });
